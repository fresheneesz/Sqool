<?php
	/*	To do:
			...
			* write sqoolAddMember and sqoolRmMember
			* write app, getElem, setElem, rmElem
	...
	*/// Modifier 'list' makes a type into a list of those types (eg "bool list, int list")
	...
	// adds a single new member to a class (a new column). 
	...
	{	// add member (and type) to the end of the members string for the given class
		...
		$member = sqoolTypeConvert(sqool_parse($memberTypeAndName));	// convert type
		sqlquery('SELECT * FROM sqool_objects WHERE class="'.$contents.'"')
		
		// loop through the sqool_objects table for rows that have "class" of $className
		{	// add the member to the object with 
			sqlquery('ALTER TABLE '..' ADD '.$member[0][0].' '.$member[0][1]);
		} 
	}
	...
	// changes an array with elements of the form (name, type, list, ref, ref list) into mySQL types
