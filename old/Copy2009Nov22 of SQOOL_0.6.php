...
{	
	...
	*/
	...
	{	$this->validateRoot("Attempting to use the backend createFetchOperation function with an object that does not represent a database.");
		...
		{	$member_className = $classDefinition[$m]["baseType"];
			...
			{	$newObject = new $member_className();
				...
				{	$member_options = $d;
					...
					$operations = array_merge($operations, $this->createFetchOperation($member_className, $member_options, $newObject, "members", $numberForVariable+1));
				}
			}
		}
		...
		(	array(array
			(	"opName" => "fetch", 
				...
				"object"=>$object, "mode"=>$mode, "numberForVariable"=>$numberForVariable
			)),
			$operations,
			array
	...
	{	return array
		(	'UPDATE '.self::getBackEndClassName($op["class"]).' SET '.implode(",", $op["queryColUpdates"])
		);
	}
	...
	{	//if(inOperationsList($op, $nonRepeatableCalls))
		...
		return array
		(	'ALTER TABLE '.$op["class"].' ADD ('.$alterQuery.')'
		);
	}
