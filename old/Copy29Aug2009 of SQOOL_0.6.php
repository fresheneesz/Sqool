		...
		class sqool			connection to a database
			...
			sclass			should return the definition for a sqool class type. This function should be defined for a class extending sqool. 
							Does not create a table in the DB until an object is inserted (lazy table creation).
								Member types: bool, string, bstring, gstring, tinyint, int, bigint, float, :class:, :type: list
...
/*	To do:
		* make sure you never use *, always select explicit names
...
{	
	...
	/*	
		...
		// OR
		rootObject->fetch("className", objectID);	// fetches a single object of a given class (the calling object must be the root object)
	...
	*/
	...
	{	if($this->queueFlag === false)
		...
		$fetchOptions = $this->membersToKeyValue($fetchOptions);
		...
		{	$this->requireID("fetch from");
			...
			$this->databaseRootObject->fetchBack(self::getFrontEndClassName($this), array
			(	"cond"=>array(self::getClassPrimaryKey(self::getFrontEndClassName($this))."=",$this->ID), 
				"ranges"=>array(0,0),	// only return the first item found (since there can only be one)
				"members"=>$fetchOptions
			),
			$this, "members", self::getFrontEndClassName($this));
		}	
	...
	{	$memberDefinition = self::getClassDefinition($op["class"]);		// members of the form array("baseType"=>baseType[, "listType" => listType])
		...
		(	"numberOfCommands" => 2,	// insert uses two sql statements
			"queries" => 'INSERT INTO `'.self::getBackEndClassName($op["class"]).'` ('.implode(",", $columns).') '.'VALUES ('.implode(",", $values).');SELECT LAST_INSERT_ID();'
		);
	}
	...
	{	
		...
		*/
		...
		{	$rowList = $results[0];
			...
			{	$info = $op["resultsInfo"][$n];
				$op["object"]->setVariables[$info["memberName"]] = array();
				self::requireDefinedClass($info["baseType"]);
	...
	{	if($op["mode"] === "tables")	// root
		{	$className = $op["resultsInfo"][count($op["resultsInfo"])-1]["memberName"];
			return $this->genericErrorHandler
			(	$op, $errorNumber, array("database", "table", "column", "noSelectedDB"), 
				$className, 
		...
		{	throw new cept("you're outta luck");
		}
	}
	...
	{	$result = $this->parseMemberSelection($op["className"], $op["options"]);
		$op["resultsInfo"] = $result["resultsInfo"];	// load some auxilary info into the operation array
	...
	{	return array
		(	"numberOfCommands" => 1,	
			"queries" => 'DROP DATABASE `'.$op["DBname"].'`;'
		);
	}
