...
{	function testUtilityMethods()
	...
	*/
	...
	{	$a = sqool::connect("root", "", "garboNonExistant");	// connect to a database that doesn't exist (connects on localhost)
		...
		self::assert($a->boo[1]->biginttest === 1);
		self::assert($a->boo[1]->floattest === 1.0);
		self::assert($a->boo[1]->doubletest === 1.0);	
	...
	{	$a = setupTestDatabase();
		...
		self::assert($booObj->moose === false);
		self::assert($booObj->barbarastreisznad === "drankFU");
		self::assert($booObj->dickweed === "lets go");
		self::assert($booObj->moose2 === "SUCKT2\" '' ~~");
		self::assert($booObj->doosh4 === 33);
		...
		self::assert($a->boo[0]->moose === false);
		self::assert($a->boo[0]->barbarastreisznad === "drankFU");
		self::assert($a->boo[0]->dickweed === "lets go");
		self::assert($a->boo[0]->moose2 === "SUCKT2\" '' ~~");
		self::assert($a->boo[0]->doosh4 === 33);
		self::assert($a->boo[0]->inttest === 44);
		self::assert($a->boo[0]->biginttest === 555);
		self::assert($a->boo[0]->floattest === 5.6665);
		self::assert($a->boo[0]->doubletest === 11.890890);
